...
**All twelve steps are in.** Thirteen commits from `bad2bac` to this one, on
`feat/language-and-currency`.

```
tests      : 250 passing, from a 153 baseline before this ticket
typecheck  : tsc --noEmit clean
lint       : eslint src clean
build      : next build clean, 11 static pages
browser    : npm run check:currency green, 21 assertions across 9 states
staging    : driven in next dev against staging-api, 2 September 2026
```
...
| Deviation | Why |
|---|---|
| **`BUILT_LOCALES` lives in `lib/locale.ts`; the component constant is gone** | Step 7. The plan said to trim `LANGUAGE_OPTIONS` in place. Half an intersection is a rule, and a rule inside a `.tsx` with `motion/react` at the top cannot be reached by `node --test` |
| **The in-flight flag is raised in `buy.ts`, not in the payment panel** | Step 8. The plan named `GetMyReading.tsx`, which holds no write — it is layout. There are two roads to a payment and both are already in one file, so the rule is stated once and a third road cannot be added that forgets it. Same argument as the row above |
| **`resolveProducts` and `useProducts` take the bundled tiles as an argument** | Step 9. They could not be tested at all otherwise: `content/home.ts` reaches for `@/lib/assets` through a path alias `node --test` does not resolve. The shape copies `resolveReadingPrice` — established four commits earlier in step 4, on this branch, not inherited from before it |
| **`data-locale-menu` on the globe** | Step 10. Neither control is in the static export, so every browser assertion has to open the panel first, and it is selected by a hook rather than by copy |
| **`chooseCurrency` drops the last resolution** | Step 12, from the review. Not in the plan, and without it the control is broken on the three pages `resolved` exists for |

### What driving it found, which nothing else would have

**Two hydration bugs, the same bug twice.** `useCatalogue` and `useProduct` both
closed over `chosen` as the hydration render reported it — and that render is
*required* to say "nothing chosen", because the export is built cold. So every
returning visitor asked cold first and asked again a moment later.

- On the homepage: two catalogue calls per reload, and detected prices painted
  for a beat before the choice corrected them. **Fact 1 was false** on that
  journey until `ab377cb`.
- On `/readings/month-ahead/`: worse, because that hook writes
  `rememberResolvedCurrency` from what comes back, so the cold answer replaced
  the visitor's stored resolution — on the page holding the checkout button.
  Fixed in `0a6bc69`, found by the Spec review rather than by the browser pass,
  which only loaded `/`.

Both are now states in `check:currency`, and both went red first there.

### Still owed

- **Fact 5's second half is unproven, and cannot be proved from here.** "The
  language group appears at two entries" cannot happen in a browser while
  `BUILT_LOCALES` is `["en"]`; no answer from `/languages` survives the
  intersection. `languages.test.ts:48` proves the resolver by varying `built`
  through the parameter it is exported with, and nothing renders either control
  with two languages. **Step 7's "a stub answering two draws both, in both
  places" is not met**, and step 10's "proves all five facts" is therefore four
  and a half. #69 is where this becomes checkable.
- **`useProduct` costs one aborted duplicate request per reload.** Its effect
  re-runs when the dependency changes and the cleanup aborts the first call.
  Removing it needs a module-scoped guard like `askCatalogue`'s; a per-mount one
  is the trap `catalogue.ts` documents, where React's development double-mount
  lets the second mount skip a request the first's cleanup aborted. Nothing
  wrong is fetched, written or painted.
- **#63 says the switcher appears "with no deploy on our side"; the intersection
  means it does not.** ADR 0004 argues the intersection and a static export
  cannot do otherwise, so this is a second contradiction with the ticket beyond
  the one the preamble above counts. Declared here rather than left for #69 to
  discover.
- **`currencies.ts` and `languages.ts` are the same store twice**, differing
  only in failure policy. Raised by the Standards review; a follow-up.
- **Currency codes are bare strings** throughout, beside `CONTEXT.md`'s **Money**
  discipline. Also raised, also larger than this ticket.

### What the vocabulary gained

`CONTEXT.md` now defines **chosen currency**, **resolved currency** and **cold**,
beside **Money**. Three where the plan named two: **cold** is the third word this
work is written in, and it is the one a later ticket is most likely to rename to
"default", which is exactly wrong.
