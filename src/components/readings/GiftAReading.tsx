 ...
 * doesn't draw, the whole panel is the link — the construction the reading
 * cards use — and it answers a pointer with `.panel-hover`, the one hover
 * definition every panel on this page shares.
 *
  ...
  return (
    <Section padding="none" className="pb-[clamp(1.5rem,2.86vw,3.4375rem)]">
      <Container width="readingsGift">
        <Link href={gift.href} className="panel-hover block no-underline">
          {/* Her mobile frame carries the crest; the desktop one doesn't. */}
          <OrnateFrame variant="panel" crest>
            ...
            <span className="flex items-stretch">
              ...
              <span className="w-[38.9%] shrink-0">
                <Image
                  src={gift.image.src}
                  alt={gift.imageAlt}
              ...
              <span className="flex w-full flex-1 flex-col items-center justify-center px-[clamp(1rem,2vw,2.5rem)] pt-[clamp(0.5rem,1.3vw,1.5625rem)] pb-[clamp(0.375rem,0.68vw,0.8125rem)] text-center">
                <h2 className="order-1 font-display text-[7.87cqw] tracking-[-0.01em] text-cream lg:text-h2-md">
                  {gift.title}
                </h2>

                {/*
                  Under the rule on the phone, above it on the desktop. She
                  sets it in a 105px box, which is what breaks it onto two
                  lines there.
                */}
