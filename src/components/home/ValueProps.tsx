...
import { valueProps, valuePropsCarousel } from "@/content/home";
    ...
    <div className="relative overflow-x-clip">
      ...
      <Section padding="none" className="relative z-10">
        <Container className="py-[clamp(2rem,7.3vw,8.75rem)]">
          <ValuePropsCarousel
            slideCount={valueProps.length}
            dotsLabel={valuePropsCarousel.dotsLabel}
            dotLabels={valueProps.map((prop) => `${valuePropsCarousel.dotAction} ${prop.title}`)}
          >
            {/*
              Column widths follow Figma, where the middle prop is wider so its title usually
              stays on one line. Below ~1400px it wraps anyway, so each slide/article pair is
              `md:contents` and the grid itself owns 4 explicit rows in `md:grid-flow-col`:
              image/title/divider/body line up as shared rows across all three columns, and a
              row's height follows its tallest cell. That keeps the dividers aligned even when
              only the middle title wraps to 2 lines. Below `sm`, `CarouselSlide` is the flex
              item the swipeable strip drags (see ValuePropsCarousel) — full width, unlike the
              product carousel's peek, so exactly one compass shows at a time.
            */}
            {valueProps.map((prop, index) => (
              <CarouselSlide key={prop.title} className="md:contents">
                <article className="mx-auto flex w-full max-w-[22rem] flex-col items-center gap-[0.3em] text-center md:contents">
                  <Image
                    src={brand.compass.src}
                    alt=""
                    width={brand.compass.width}
                    height={brand.compass.height}
                    className="compass-breathe mx-auto w-[clamp(6rem,9.9vw,11.875rem)]"
